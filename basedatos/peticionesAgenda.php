...
if (isset($_SESSION['user_id'])) {
    ...
    switch ($opcion) {
        case 1: //select
            $consulta = $conexion->prepare("SELECT dia from citas where dni_Prof='$DNIProf' and dia>'$hoy' GROUP by dia order by dia");
            $consulta->execute();
        ...
        case 2: //insert
            ...
            if ($numF>0) {
                ...
                $consulta = $conexion->prepare("insert into citas(hora,dia,dni_Prof,id_cliente) values('$hora','$dia','$DNIProf','$id[0]')");
                $consulta->execute();
            } else {
        ...
        break;
        case 4: //datosCitas
            $consulta = $conexion->prepare("SELECT * from citas,clientes where dni_Prof='$DNIProf' and citas.id_cliente=clientes.id_cliente and dia='$dia' order by citas.hora");
            $consulta->execute();
            $data = $consulta->fetchAll(PDO::FETCH_ASSOC);
        break;
    }
